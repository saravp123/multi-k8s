...
docker build -t saravp123/multi-worker:latest -t saravp123/multi-worker:$SHA -f ./server/Dockerfile ./worker
...
kubectl set image deployments/client-deployment worker=saravp123/multi-worker:$SHA